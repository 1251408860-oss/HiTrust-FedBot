...
This repository contains the reviewer-facing artifact package for the HiTrust-FedBot study on hierarchical and trust-aware federated Web bot detection under congested edge environments. The current evidence base spans topology-aware pilot scenarios, same-task public Ca-Bench validation on `scenario_e` and `scenario_h`, an FLTrust-like baseline, and an auxiliary public NSL-KDD validation path.
...
| `data_hitrust/` | Released derived internal graphs, same-task public Ca-Bench graphs, and auxiliary NSL-KDD graphs |
...
The public release supports paper-level reruns from the released derived graphs, reruns of the same-task public Ca-Bench `scenario_e` and `scenario_h` paths, reruns of the auxiliary public NSL-KDD path, and reruns of the conditional hardening package. It does not support rebuilding the internal pilot graphs from upstream private raw collection traces. That boundary is intentional and is documented consistently in `RELEASE_CHECKLIST.md`, `docs/DATA_AVAILABILITY_20260324.md`, and `docs/ARTIFACT_STATUS_20260326.md`.
